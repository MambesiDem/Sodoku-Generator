...
//        return IntStream.range(0, 9).mapToObj((row) -> IntStream.range(0, 9)
//                .filter((col) -> !this.grid.isFact(row, col) && this.grid.getBox(row, col).getNumberOfPossibilities() > 0)
//                .mapToObj((col) -> this.grid.getBox(row, col)).collect(Collectors.toList()))
